...
**[anyonce](https://github.com/sns45/anyonce)** — One idempotency core behind three ingress doors: HTTP `Idempotency-Key` endpoints, anyq queue consumers, and Standard Webhooks receivers, in TypeScript and Go. Each key is claimed in **one atomic write** on every store (memory, Durable Objects, D1, DynamoDB, Redis, Postgres, SQLite), with the SQL, Lua, and DynamoDB claim statements byte-identical across the two languages and a race of 50 concurrent claims run on every backend; over HTTP, a duplicate gets the stored result back, a concurrent copy gets 409, a changed payload gets 422. Ships an executable conformance suite for the IETF `Idempotency-Key` draft and has graded three other implementations with it (core vectors: hono-idempotency 0.9.1 11/11, idempo v1.0.0 9/11, Fiber v3.5.0 7/11); the receiving twin of anyhook, 0.1.0 on npm and as a tagged Go module.
